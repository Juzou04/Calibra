...
### P4 · dificultad 2
Calcula $\iint_R xy\,dA$ donde R es el rectángulo [0,2] × [0,1]. Al integrar primero respecto a x:
...
### P5 · dificultad 2
Al convertir $\iint_R f(x,y)\,dA$ a coordenadas polares donde R es el círculo x² + y² ≤ 4, los límites correctos son:
...
### P6 · dificultad 2
Al calcular $\iint_R e^{x^2+y^2}\,dA$ sobre el círculo unitario usando coordenadas polares, el elemento de área correcto es:
...
### P7 · dificultad 2
Calcula $\int_C y\,dx$ donde C es el segmento de (0,0) a (1,1) parametrizado como r(t) = (t, t) con 0 ≤ t ≤ 1:
...
### P8 · dificultad 2
Para calcular $\int_C F\cdot dr$ donde F = (y, -x) y C es el círculo unitario recorrido en sentido antihorario, la parametrización correcta es:
...
### P9 · dificultad 2
Si F = ∇f es un campo conservativo y C es una curva de A a B, entonces $\int_C F\cdot dr$ es igual a:
...
### P10 · dificultad 2
El teorema de Green permite calcular $\int_C P\,dx + Q\,dy$ como una integral doble sobre la región D cuando:
...
### P11 · dificultad 2
Para aplicar el teorema de Stokes a $\int_C F\cdot dr$ donde C es la frontera de una superficie S:
...
### P12 · dificultad 2
El teorema de Gauss (divergencia) relaciona $\iint_S F\cdot n\,dS$ con:

- A) $∭_V \mathrm{div}\,F\,dV$ donde S encierra V · CORRECTA
- B) $\iint_S \mathrm{rot}\,F\cdot n\,dS$ · usas Stokes donde corresponde Gauss
- C) $\int_C F\cdot dr$ donde C es la frontera de S · aplicas teorema incorrecto para superficies cerradas
